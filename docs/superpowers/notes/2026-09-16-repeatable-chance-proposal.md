...
- **Many tries (hundreds) at a low chance each** - mine stones. You break hundreds of stones a
  day, so even a fraction of a percent per stone adds up.
- **Dozens of tries at a real chance each** - monster kills and fishing casts. Here the per-try
  number has to be a meaningful slice, not a lottery ticket.
...
|---|---|---|---|---|---|
| Mine node rows in `MineSources.NodeTable` marked Chance: coal, amethyst, topaz, jade, aquamarine, ruby, emerald, geode, frozen geode, magma geode, omni geode | `MineShaft.cs` 3642 (geode/frozen geode/magma geode, 2.2% per stone), 3656 (omni geode, 0.5% per stone), 3665+3669 (coal, 5% ore roll x 25% coal-within-that = ~1.25% per stone), 4612 (gem nodes, ~0.3% per stone, base `gemStoneChance = 0.003` set at 1307) | hundreds of stones (200+ on a mining day) | 0.3% to 2.2% per stone (coal ~1.25%) | roughly 45% to over 99%, depending on the gem | **PROMOTE** |
| Diamond node | `MineShaft.cs` 4607: `0.00025 + mineLevel/120000 + ...`, floor 51+ only | hundreds of stones, very rare node | about 0.03% at floor 51, up to about 0.15% by floor 120 | well under 1% even with 200 stones (roughly 25% to 30%) | **KEEP** - stays a lottery ticket even with hundreds of tries |
| Monster drops (`MineSources.MonsterDrops`), from `Data/Monsters` field 6 | data-driven, not code; the model reads `row.Chance` directly (`GameObtainabilityData.cs:151`) | dozens of kills | varies by monster/item; promote at `chance >= 0.25` | at 0.25 chance and 20 kills, about 99.7% | **PROMOTE when `row.Chance >= 0.25`** |
| Fishing trash (`SpawnSources.FishingTrash`) | `GameLocation.cs` 13685 (driftwood is the universal fallback when no fish spawn resolves) and `MineShaft.cs` 1188-1197 (mine fishing returns trash directly on almost every cast below the lava lake floor, and at 95% odds even in the lava lake); confirmed in game by the `fish-catch-rates-2026-09-04.md` simulation, which measured roughly a quarter to a third of casts landing trash for a level-10 angler with bait | about 30 casts on a fishing day (measured: closer to 10-20 realistic catches once bite time and the bobber minigame are counted, see the same note) | roughly 15-33% per cast, and up to 100% while fishing in most mine areas | at even the low end (15%, 15 casts), about 91% | **PROMOTE** |
