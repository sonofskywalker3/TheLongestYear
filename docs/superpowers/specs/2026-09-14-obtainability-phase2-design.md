...
| Mine depth in the landing week (2026-09-16) | n/a | floors recorded as a condition only | Jeff: the report must be as accurate as possible, so a mine-floor route lands after the days it takes to get there at 10 floors a day from nothing (floor N: ceil(N/10) days). The darkness check credits the depth the save has already reached, so nothing is counted twice. Skull Cavern routes get no travel days: the bus and Staircase conditions already price them. |
