...
namespace TheLongestYear
{
    public sealed class ModEntry : Mod
    {
        ...
        private void CmdDumpAvailability(string command, string[] args)
        {
            ...
            sb.AppendLine();
            sb.AppendLine("**Earliest** is the first season the engine says the item can exist at all. It is a hard floor only when the model DERIVED it (fish, crab-pot, metals, with location gating: the mines and the Sewer open in Summer, the Desert and Skull Cavern in Fall). An item marked *effort-only* has no derived floor: the gate treats it as Winter (the safe direction, so a deadline is never impossible), while the weekly goals use the catalog seasons. **Catalog seasons** are the spawn seasons the bundle catalog assigned (crops, forage, fish; `any` = year-round). **Due** is the season the day-28 gate demands the item (per-item pin), the bundle's season (seasonal), or the quota ramp (pick-X-of-Y, no per-item date).");
            sb.AppendLine();
            foreach (BundleRequirement req in requirements)
            {
                ...
                sb.AppendLine();
                sb.AppendLine("| Item | Id | Earliest | Floor | Catalog seasons | Due | Effort | Basis |");
                sb.AppendLine("|---|---|---|---|---|---|---|---|");
                foreach (string id in req.Ingredients)
                {
                    TheLongestYear.Core.ItemAvailability a = _availability.For(id);
                    bool derived = _availability.IsDerived(id);
                    ...
                    sb.AppendLine($"| {DisplayName(id)} | {id} | {a.EarliestSeason} | {(derived ? "derived" : "effort-only")} | {catalogSeasons} | {due} | {a.Effort} ({a.Source}) | {a.Basis.Replace("|", "/")} |");
                }
                sb.AppendLine();
            }
            ...
            this.Monitor.Log($"tly_dumpavailability: wrote {path} ({sb.Length:N0} chars, {requirements.Count} bundles).", LogLevel.Info);
        }
