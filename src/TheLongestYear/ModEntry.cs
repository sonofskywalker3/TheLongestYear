...
namespace TheLongestYear
{
    public sealed class ModEntry : Mod
    {
        ...
        private void CmdDumpAvailability(string command, string[] args)
        {
            ...
            sb.AppendLine();
            sb.AppendLine("**Week** is the first week of the year (1 to 16) the engine says the item can exist; **Gate** the season a day-28 gate may first demand it (the deep mine and the Skull Cavern gate later than their goal week). **Placed** says who decided: `derived` (fish, crab-pot, metals from game data), `rule` (a Phase 2 rule: mines, geodes, monsters, artifacts, animals, machines, dishes, ponds, crops, forage, saplings), `override` (a pin or AvailabilityWeekOverrides), or `UNKNOWN` (nothing placed it: the gate treats it as Winter and it is listed at the end for Jeff to rule on). **Catalog seasons** are the spawn seasons the bundle catalog assigned (`any` = year-round). **Due** is the season the day-28 gate demands the item (per-item pin), the bundle's season (seasonal), or the quota ramp (pick-X-of-Y).");
            sb.AppendLine();
            ...
            foreach (BundleRequirement req in requirements)
            {
                ...
                sb.AppendLine();
                sb.AppendLine("| Item | Id | Week | Gate | Placed | Catalog seasons | Due | Effort | Basis |");
                sb.AppendLine("|---|---|---|---|---|---|---|---|---|");
                foreach (string id in req.Ingredients)
                {
                    TheLongestYear.Core.ItemAvailability a = _availability.For(id);
            ...
            this.Monitor.Log($"tly_dumpavailability: wrote {path} ({sb.Length:N0} chars, {requirements.Count} bundles, {unknown.Count} unknown item(s)).", LogLevel.Info);
        }
