...
namespace TheLongestYear.UI
{
    ...
    internal sealed class CraftbookMenu : IClickableMenu
    {
        ...
        private string Notice => _subtitle
            ?? (RecipeBanking.IsOverCap(_slotCount, _meta.CraftbookRecipes.Count) ? Strings.Get("menu.books.over-cap") : null);
        private int HeaderHeight => Notice == null ? 80 : 120;

        /// <summary>Rows drawn: every slot plus any grandfathered overflow.</summary>
        private int RowCount => RecipeBanking.VisibleRows(_slotCount, _meta.CraftbookRecipes.Count);
            ...
            : base(0, 0, 0, 0, showUpperRightCloseButton: true)
        {
            ...
            _monitor.Log(
                ...
                $"overCap={RecipeBanking.IsOverCap(_slotCount, meta.CraftbookRecipes.Count)}.",
                LogLevel.Info);
            RecomputeLayout();
        ...
        private void BankRecipe(string recipeId)
        {
            ...
            if (!RecipeBanking.CanBank(_slotCount, _meta.CraftbookRecipes.Count))
            {
                // No free slot (full, or over the cap after a tier ladder change): refuse.
                _monitor.Log($"CraftbookMenu: refused '{recipeId}', slots={_slotCount}, banked={_meta.CraftbookRecipes.Count}.", LogLevel.Info);
        ...
        private void PromptRemove(int slotIndex)
        {
            ...
            Game1.activeClickableMenu = new ConfirmationDialog(
                Strings.Get("menu.craftbook.remove-confirm", new Dictionary<string, string> { ["recipe"] = name }),
                _ =>
                {
