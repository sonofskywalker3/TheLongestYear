...
{
    [Fact]
    public void No_upgrades_excludes_all_three()
    {
        var ex = YearTwoCrops.ExcludedFor(_ => false);
        Assert.Equal(new[] { "(O)248", "(O)266", "(O)274" }, ex.OrderBy(x => x, System.StringComparer.Ordinal));
    }

    [Fact]
    public void Red_cabbage_upgrade_frees_only_red_cabbage()
    {
        var ex = YearTwoCrops.ExcludedFor(id => id == YearTwoCrops.RedCabbageUpgrade);
    ...
    [Fact]
    public void Pierre_upgrade_frees_everything()
        => Assert.Empty(YearTwoCrops.ExcludedFor(id => id == YearTwoCrops.PierreUpgrade));

    [Fact]
    public void Extra_excluded_ids_keep_crops_out_of_the_pools()
    {
        var pools = ItemPoolBuilder.Build(
            ...
            extraExcludedIds: YearTwoCrops.ExcludedFor(_ => false));
