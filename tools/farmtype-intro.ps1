...
if ($SkipIntro) {
    ...
    # event fires on its own vanilla precondition (60367/u 0) right away, since vanilla's
    # checkForEvents runs every frame regardless of the mod's own activation state. SMAPI's
    # Context.IsWorldReady (and so RunActivation.IsActive, and the mod's own SaveLoaded/"Run N
    # ready" line) stays false until dayOfMonth leaves 0, which only happens once the event's
    # "end beginGame" runs. The event necessarily comes BEFORE "Run N ready" and the stash/shrine
    # placements, not after, and nothing the mod itself logs can mark "the event started" (every
    # mod driver is gated on RunActivation, which isn't true yet either). Detect it from the
    # eventstep loop instead: the first real cmd[N] (as opposed to "no event") is the start.
    ...
    # index, text, actors and location, run `debug ee`, then `tly_totitle`, and report
    # DONE_WITH_CONCERNS instead of patching the script.
