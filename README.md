...
- **Friendship 101 is no longer in the Book bundle.** It is the ninth prize from the prize ticket machine, about 25 Help Wanted quests, and the ticket count starts over every loop, so no one reaches it inside a year. On Hard the Book bundle wants four of its five books, so that one book made the season unwinnable. The Alleyway Buffet and Mapping Cave Systems stay: the Bookseller sells them in year 3, but each sits in a free one-time gift box (behind Joja in Town, and in the Adventurer's Guild) that comes back every loop. Reported by FayGabi.
